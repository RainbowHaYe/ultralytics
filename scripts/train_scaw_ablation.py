...
实验设计：
  基于 SCAW 三项创新 (S=SPDLiteConv, C=CoordAtt, W=WAFF)，
...
数据集  ：selfpart499_augmented (7 类零件)
硬件环境：NVIDIA A100-PCIE-40GB, CUDA 12.6

用法：
  # 运行全部 9 组实验（按序）
  python scripts/train_scw_ablation.py

  # 仅运行指定实验（逗号分隔编号）
  python scripts/train_scw_ablation.py --experiments 0,2,8

  # 仅运行指定实验（名称）
  python scripts/train_scw_ablation.py --experiments baseline,SCW

  # 从第 5 组开始继续（跳过已完成的）
  python scripts/train_scw_ablation.py --start-from 5

结果路径：results/runs/self499_aug/<name>/
==============================================================================
"""
...
DATA_YAML = "datasets/selfpart499_augmented/data.yaml"

# ============================================================================
# 公共训练超参数（与 baseline 脚本一致，确保消融实验的公平性）
# ============================================================================
TRAIN_ARGS = dict(
    # ── 数据与任务 ──
    data=DATA_YAML,
...
def parse_args():
    """解析命令行参数。"""
    parser = argparse.ArgumentParser(description="YOLO-SCAW 消融实验训练")
    parser.add_argument(
        "--experiments", type=str, default=None,
        help="要运行的实验，逗号分隔。支持编号(0-8)或名称(baseline,SCW等)。"
             "默认运行全部 9 组。"
    )
    parser.add_argument(
        "--start-from", type=int, default=0,
        help="从第几组实验开始 (0-8)，用于中断后继续"
    )
    parser.add_argument(
        "--project", type=str, default="results/runs/self499_aug",
        help="实验结果保存的项目目录"
    )
...
def run_experiment(exp_id, exp_name, model_src, description, project):
    ...
    # ------------------------------------------------------------------
    # 清理：释放 GPU 显存和 CPU 内存，防止下一组实验 OOM / 卡死
    # ------------------------------------------------------------------
    del model, results
    gc.collect()
...
def print_summary_table(all_results):
    ...
    print("=" * 100)
    print("  数据集: selfpart499_augmented (7 类)")
